		...
		<ul class="nav navbar-nav navbar-right">
			...
			<li class="custom-nav">
				...
				<li class="cart">
					...
					<a href="#" onclick="getCart();"><i class="fa fa-shopping-cart" aria-hidden="true"></i> <span class="badge" id="cart_contents"><?php echo $cart_count ;?></span></a>
					...
					<?php
						...
						if(!empty($_SESSION["cart_array"])){
							...
							foreach($_SESSION["cart_array"] as $eachItem){
								...
								$shopping_cart .= '<div class="col-md-3 col-xs-4"><a href="'.base_url().'collections/product/'.strtolower(html_escape($item_id)).'/'.url_title(strtolower(html_escape($productName))).'" title="'.ucwords(html_escape($productName)).'"><div class="image-container">'.$thumbnail.'<div class="image-overlay"></div></div></a></div>';
								$shopping_cart .= '<div class="col-md-7 col-xs-6">';
								$shopping_cart .= '<h5><a href="'.base_url().'collections/product/'.strtolower(html_escape($item_id)).'/'.url_title(strtolower(html_escape($productName))).'" title="'.ucwords(html_escape($productName)).'">'.ucwords(html_escape($productName)).'</a>';
								$shopping_cart .= '</h5>';
								$shopping_cart .= '<p><span class="small">'.$product_qty.'</span> X <span class="item-price">$'.$price.'</span></p>';
								$shopping_cart .= '</div>';
								$shopping_cart .= '<div class="col-md-2 col-xs-2"><span class="pull-right"><a href="#" title="Remove Item" onclick="removeItem(this,'.$i.','.$item_id.');" class="remove-item" id="'.$i.'-'.$item_id.'"><i class="fa fa-trash" aria-hidden="true"></i></a></span></div>';
								$shopping_cart .= '</div>';
